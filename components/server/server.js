...
app.get('/books', async (req, res) => {
    try{
        ...
        const response = await fetch(`https://api.nytimes.com/svc/books/v3/lists/current/graphic-books-and-manga.json?api-key=${API_KEY}`);

        const data = await response.json();

        // breaking through layers
        const books = data.results.books.slice(0,15);
